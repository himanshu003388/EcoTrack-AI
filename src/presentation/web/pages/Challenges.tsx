...
import { useAuth, AppUser, apiFetch } from '../main';
...
export const Challenges: React.FC = memo(() => {
  ...
  usePageTitle('Eco Challenges & Achievements');
  ...
  const fetchChallengesAndBadges = async (): Promise<void> => {
    try {
      ...
      // Non-critical: user will see empty state
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleComplete = async (id: number): Promise<void> => {
    try {
      const res = await apiFetch(`/api/challenges/${id}/complete`, {
        method: 'POST',
        headers: token !== null ? { Authorization: `Bearer ${token}` } : undefined,
      });
      const data = (await res.json()) as { error?: string };
  ...
  const badgeCabinet = [
    {
      ...
      check: (u: AppUser): boolean => u.points >= 10,
    },
    {
      id: 2,
      name: 'Consistent Tracker',
      desc: 'Maintain a 3-day log streak',
      icon: Flame,
      iconColor: 'text-amber-500',
      check: (u: AppUser): boolean => u.streak >= 3,
    },
    {
      id: 3,
      name: 'Streak Veteran',
      desc: 'Maintain a 7-day log streak',
      icon: Zap,
      iconColor: 'text-yellow-500',
      check: (u: AppUser): boolean => u.streak >= 7,
    },
    {
      id: 4,
      name: 'Sapling Status',
      desc: 'Reach the Sapling XP level',
      icon: Sprout,
      iconColor: 'text-forest-500',
      check: (u: AppUser): boolean => u.points >= 101,
    },
    {
      id: 5,
      name: 'Green Activist',
      desc: 'Earn 500 environmental points',
      icon: Shield,
      iconColor: 'text-emerald-600',
      check: (u: AppUser): boolean => u.points >= 500,
    },
    {
      id: 6,
      name: 'Climate Champion',
      desc: 'Reach Climate Hero status',
      icon: Crown,
      iconColor: 'text-purple-500',
      check: (u: AppUser): boolean => u.points >= 1001,
    },
  ];
  ...
  const getNextLevelInfo = (points: number): { next: string; needed: number; progress: number } => {
    ...
    if (points >= 601)
      return { next: 'Climate Hero', needed: 1001 - points, progress: Math.round(((points - 600) / 400) * 100) };
    if (points >= 301)
      return { next: 'Forest Guardian', needed: 601 - points, progress: Math.round(((points - 300) / 300) * 100) };
    if (points >= 101)
      return { next: 'Tree', needed: 301 - points, progress: Math.round(((points - 100) / 200) * 100) };
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <section
        className="bg-white dark:bg-forest-900 border border-slate-100 dark:border-forest-800 p-8 rounded-[32px] shadow-sm grid grid-cols-1 md:grid-cols-3 gap-8 items-center"
        aria-label="Points progress card"
      >
        ...
        <div className="space-y-2 md:col-span-2">
          ...
          <div
            className="w-full bg-slate-100 dark:bg-forest-800 h-3.5 rounded-full overflow-hidden"
            role="progressbar"
            aria-valuenow={nextLevel.progress}
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label={`Level progress: ${nextLevel.progress} percent`}
          >
            <div
          ...
          <div
            className="flex justify-between text-[10px] font-bold text-slate-400 dark:text-slate-500"
            aria-hidden="true"
          >
            <span>Seedling</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6" role="list" aria-label="Active challenge cards">
            {activeChallenges.map((ch) => {
              ...
              return (
                <div
                  key={ch.challengeId}
                  className="p-6 bg-white dark:bg-forest-900 border border-slate-100 dark:border-forest-800 rounded-3xl shadow-sm flex flex-col justify-between gap-4"
                  role="listitem"
                >
                  <div className="space-y-1">
                    ...
                    <h4 className="font-display font-bold text-slate-800 dark:text-slate-200 text-base mt-1">
                      {ch.title}
                    </h4>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-xs font-semibold text-slate-500 dark:text-slate-400">
                      ...
                      <span>
                        {ch.progress} / {ch.durationDays} days
                      </span>
                    </div>
                    <div
                      className="w-full bg-slate-50 dark:bg-forest-800 border border-slate-100 dark:border-forest-700 h-2.5 rounded-full overflow-hidden"
                      role="progressbar"
                      aria-valuenow={progressPct}
                      aria-valuemin={0}
                      aria-valuemax={100}
                      aria-label={`Challenge progress: ${progressPct} percent`}
                    >
                      <div
                  ...
                  <div className="flex justify-between items-center pt-2">
                    <span className="text-xs font-bold text-slate-500 dark:text-slate-400">
                      Reward: +{ch.pointsReward} XP
                    </span>
                    <button
                      onClick={() => {
                        void handleComplete(ch.challengeId);
                      }}
      ...
      <section aria-label="Available challenges" className="space-y-4">
        ...
        <div
          ...
          role="list"
          aria-label="Available challenge cards"
        >
          {challenges
            ...
            .map((ch) => (
              <div
                key={ch.id}
                className="p-6 bg-white dark:bg-forest-900 border border-slate-100 dark:border-forest-800 rounded-3xl shadow-sm hover:shadow-md transition-all flex flex-col justify-between min-h-[200px]"
                role="listitem"
              >
                <div className="space-y-1">
                  ...
                  <h4 className="font-display font-bold text-slate-800 dark:text-slate-200 text-sm mt-1.5">
                    {ch.title}
                  </h4>
        ...
        >
          {badgeCabinet.map((badge) => {
            ...
            return (
              ...
              >
                <span
                  className={`block ${isEarned ? badge.iconColor : 'text-slate-300 dark:text-slate-600'}`}
                  aria-hidden="true"
                >
                  <badge.icon className="h-10 w-10" strokeWidth={1.5} />
                </span>
                <div className="mt-3">
                  <span className="block font-display font-bold text-xs text-slate-800 dark:text-slate-200">
                    {badge.name}
                  </span>
                  <span className="text-[10px] text-slate-400 dark:text-slate-500 font-medium mt-1 leading-snug block">
                    {badge.desc}
                  </span>
                </div>
                {isEarned ? (
