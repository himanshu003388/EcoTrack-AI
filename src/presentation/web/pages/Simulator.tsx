...
export const Simulator: React.FC = memo(() => {
  ...
  const carCostPerKm = 0.20;
  const transitCostPerKm = 0.08;
  const meatMealCost = 8.50;
  const vegMealCost = 5.00;
  ...
  const chartData = useMemo(() => [
    { name: 'Baseline weekly footprint', value: Math.round(baseFootprints.total), fill: '#94a3b8' },
    { name: 'Simulated weekly footprint', value: Math.round(simData.totalFootprint), fill: '#1E3F20' }
  ], [baseFootprints.total, simData.totalFootprint]);
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6" role="region" aria-label="Adjustable habit sliders">
          ...
          <div className="bg-white dark:bg-forest-900 border border-slate-100 dark:border-forest-800 p-6 rounded-[32px] shadow-sm min-h-[250px] flex flex-col justify-between transition-colors duration-200" role="region" aria-label="Weekly footprint comparison chart">
            <h3 className="font-display text-lg font-bold text-slate-900 dark:text-white">Weekly Footprint Comparison (kg CO₂e)</h3>
            <div className="h-44 mt-4" role="img" aria-label={`Bar chart comparing baseline footprint ${Math.round(baseFootprints.total)} kg versus simulated footprint ${Math.round(simData.totalFootprint)} kg`}>
        ...
        <div className="space-y-6" role="region" aria-label="Simulation results">
          <div className="bg-white dark:bg-forest-900 border border-slate-100 dark:border-forest-800 p-6 rounded-[32px] shadow-sm space-y-4 transition-colors duration-200">
            <h3 className="text-slate-500 dark:text-slate-400 text-xs font-semibold uppercase tracking-wider">Simulated Weekly Footprint</h3>
            <div className="flex items-baseline gap-2">
              <span className="text-5xl font-extrabold text-slate-900 dark:text-white font-display" aria-label={`${Math.round(simData.totalFootprint)} kilograms CO2 per week`}>
            ...
            <div className="pt-2">
              <div className="flex justify-between text-xs font-bold text-slate-500 dark:text-slate-400 mb-1">
                ...
                <span className={simData.totalFootprint <= sustainableTargetWeekly ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'}>
                  {Math.round(simData.totalFootprint)} / 38.5 kg
                </span>
              </div>
              <div className="w-full bg-slate-100 dark:bg-forest-800 h-2.5 rounded-full overflow-hidden" role="progressbar" aria-valuenow={Math.min(100, (simData.totalFootprint / 150) * 100)} aria-valuemin={0} aria-valuemax={100} aria-label="Progress towards sustainable target">
                <div
          ...
          <div className="p-6 bg-forest-500 text-white rounded-[32px] shadow-md space-y-6 relative overflow-hidden">
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-extrabold font-display" aria-label={`${Math.round(simData.carbonSaved)} kilograms CO2 saved per week`}>{Math.round(simData.carbonSaved)}</span>
                <span className="text-xs text-forest-100 font-semibold">kg CO₂e / week saved ({carbonReductionPct}%)</span>
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-3xl font-extrabold font-display" aria-label={`$${Math.round(simData.moneySaved)} USD saved per week`}>${Math.round(simData.moneySaved)}</span>
            ...
            <div className="space-y-2 pt-2 border-t border-forest-400/50">
              ...
              <div className="w-full bg-forest-700/60 h-2 rounded-full overflow-hidden" role="progressbar" aria-valuenow={goalAchievementProgress} aria-valuemin={0} aria-valuemax={100} aria-label="Goal alignment progress">
                <div
          ...
          <div className="bg-white dark:bg-forest-900 border border-slate-100 dark:border-forest-800 p-6 rounded-[32px] shadow-sm space-y-4 text-xs font-semibold text-slate-700 dark:text-slate-300 transition-colors duration-200" role="region" aria-label="Dynamic simulation insights">
            ...
            <div className="space-y-2.5 leading-relaxed text-slate-600 dark:text-slate-400 font-medium">
              {car < 150 && (
                <p className="flex gap-2">
                  ...
                  Reducing your weekly driving to {car} km prevents approximately {Math.round((baseCar - car) * carFactor)} kg of tailpipe carbon.
                </p>
              )}
              {meat < 3 && (
                <p className="flex gap-2">
                  <span className="text-emerald-500 shrink-0" aria-hidden="true">✔</span>
                  Limiting meat intake to {meat} days saves ${Math.round((baseMeat - meat) * (meatMealCost - vegMealCost))} in meal shopping expenses weekly.
                </p>
              )}
              {energy < 70 && (
                <p className="flex gap-2">
                  <span className="text-emerald-500 shrink-0" aria-hidden="true">✔</span>
                  Lowering house electricity to {energy} kWh reduces power grid demand, preventing {Math.round((baseEnergy - energy) * electricityFactor)} kg CO₂.
                </p>
              )}
          ...
          <div className="bg-white dark:bg-forest-900 border border-slate-100 dark:border-forest-800 p-6 rounded-[32px] shadow-sm space-y-4 transition-colors duration-200" role="region" aria-label="Your personalized reduction action plan">
            <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider flex items-center gap-1.5">
              <Lightbulb className="h-4 w-4 text-amber-500" aria-hidden="true" />
              Your Action Plan
            </h4>
            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium">Personalized recommendations to reduce your weekly footprint:</p>
            <ol className="space-y-2.5 list-none">
              {car > 120 && (
                <li className="flex gap-2.5 items-start p-2.5 bg-slate-50 dark:bg-forest-800/40 rounded-xl" role="listitem">
                  <span className="flex h-5 w-5 items-center justify-center rounded-full bg-forest-500 text-white text-[10px] font-bold shrink-0 mt-0.5" aria-hidden="true">1</span>
                  <div>
                    <p className="text-xs font-bold text-slate-700 dark:text-slate-300">Reduce driving to 120 km/week</p>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Saves ~{Math.round((car - 120) * carFactor)} kg CO₂ and ${Math.round((car - 120) * carCostPerKm)} weekly. Try carpooling or remote work.</p>
                  </div>
                </li>
              )}
              {transit < 150 && (
                <li className="flex gap-2.5 items-start p-2.5 bg-slate-50 dark:bg-forest-800/40 rounded-xl" role="listitem">
                  <span className="flex h-5 w-5 items-center justify-center rounded-full bg-forest-500 text-white text-[10px] font-bold shrink-0 mt-0.5" aria-hidden="true">{car > 120 ? '2' : '1'}</span>
                  <div>
                    <p className="text-xs font-bold text-slate-700 dark:text-slate-300">Increase public transit to 150 km/week</p>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Transit emits ~60% less CO₂ per km than cars. Cheaper than fuel too!</p>
                  </div>
                </li>
              )}
              {meat > 2 && (
                <li className="flex gap-2.5 items-start p-2.5 bg-slate-50 dark:bg-forest-800/40 rounded-xl" role="listitem">
                  <span className="flex h-5 w-5 items-center justify-center rounded-full bg-forest-500 text-white text-[10px] font-bold shrink-0 mt-0.5" aria-hidden="true">{(car > 120 ? 1 : 0) + (transit < 150 ? 1 : 0) + 1}</span>
                  <div>
                    <p className="text-xs font-bold text-slate-700 dark:text-slate-300">{meat > 3 ? 'Cut meat to 2 days/week' : 'Cut meat to 1 day/week'}</p>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Each meat-free day saves ~4.6 kg CO₂e. Try a{" "}
                      <a href="/tracker" className="text-forest-600 dark:text-forest-400 underline">vegetarian meal today</a>.
                    </p>
                  </div>
                </li>
              )}
              {energy > 50 && (
                <li className="flex gap-2.5 items-start p-2.5 bg-slate-50 dark:bg-forest-800/40 rounded-xl" role="listitem">
                  <span className="flex h-5 w-5 items-center justify-center rounded-full bg-forest-500 text-white text-[10px] font-bold shrink-0 mt-0.5" aria-hidden="true">{(car > 120 ? 1 : 0) + (transit < 150 ? 1 : 0) + (meat > 2 ? 1 : 0) + 1}</span>
                  <div>
                    <p className="text-xs font-bold text-slate-700 dark:text-slate-300">Lower energy use to 50 kWh/week</p>
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Saves ~{Math.round((energy - 50) * electricityFactor)} kg CO₂ weekly. Switch to LED bulbs and unplug standby devices.</p>
                  </div>
                </li>
              )}
              {car <= 120 && transit >= 150 && meat <= 2 && energy <= 50 && (
                <li className="flex gap-2 items-start p-2.5 bg-emerald-50 dark:bg-emerald-950/30 rounded-xl">
                  ...
                  <div>
                    <p className="text-xs font-bold text-emerald-700 dark:text-emerald-300">Looking good! Your current settings are near sustainable targets.</p>
                    <p className="text-[10px] text-emerald-600 dark:text-emerald-400 mt-0.5">Challenge others or set a new goal in the <a href="/challenges" className="underline">Challenges</a> page.</p>
                  </div>
                </li>
              )}
            </ol>
