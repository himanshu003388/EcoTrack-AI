...
import { useBeforeUnload } from '../components/Hooks';
import {
  Car, Flame, Utensils, ShoppingBag, Plus, Search,
  RefreshCw, Trash2, HelpCircle, Sparkles, Lightbulb
} from 'lucide-react';
...
export const Tracker: React.FC = () => {
  ...
  const subcategoryConfig: Record<string, { name: string; unit: string; options: string[] }> = {
    transport: {
      name: 'Transportation',
      unit: 'km',
  ...
  const smartSuggestions = [
    ...
    { label: 'Online Shopping Delivery', category: 'shopping_waste', subcategory: 'deliveries', quantity: 1, unit: 'kg' },
    { label: 'Plastic Recycling (2.5 kg)', category: 'shopping_waste', subcategory: 'recycling', quantity: 2.5, unit: 'kg' }
  ];
  ...
  useEffect(() => {
    ...
    else if (category === 'shopping_waste') setQuantity(2);
  ...
  const getCategoryIcon = (catName: string) => {
    switch (catName) {
      ...
      case 'food': return <Utensils className="h-5 w-5" aria-hidden="true" />;
      default: return <ShoppingBag className="h-5 w-5" aria-hidden="true" />;
    }
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div>
        <h1 className="font-display text-3xl font-bold tracking-tight text-slate-900 dark:text-white text-balance">Activity Tracker</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6" role="region" aria-label="Log activity form">
          <div className="bg-white dark:bg-forest-900 border border-slate-100 dark:border-forest-800 p-6 rounded-[28px] shadow-sm flex flex-col gap-5 transition-colors duration-200">
            ...
            <div className="grid grid-cols-4 gap-2" role="tablist" aria-label="Activity category">
            ...
            <div id="category-panel" className="space-y-4" role="tabpanel" aria-label={`${subcategoryConfig[category].name} form`}>
              <form id="category-panel-form" onSubmit={handleSubmit}>
              <div>
                <label htmlFor="subcategory" className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">
                  Subcategory
                </label>
                <select
                  id="subcategory"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="quantity" className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">
                    Quantity
                  </label>
                  <input
                    id="quantity"
                    ...
                    step="any"
                    value={quantity}
                    onChange={(e) => { setQuantity(parseFloat(e.target.value) || 0); setQuantityError(null); }}
                    className={`block w-full px-3 py-2.5 border rounded-xl text-sm focus:outline-none focus:ring-1 tabular-nums bg-white dark:bg-forest-800 text-slate-900 dark:text-slate-100 ${
                      quantityError ? 'border-red-300 dark:border-red-700 focus:border-red-500 focus:ring-red-500' : 'border-slate-200 dark:border-forest-700 focus:border-forest-500 focus:ring-forest-500'
                    }`}
                    required
                    aria-required="true"
                  ...
                  />
                  {quantityError && (
                    <p id="quantity-error" className="mt-1 text-xs text-red-500 font-medium" role="alert">{quantityError}</p>
                  )}
                </div>
                <div>
                  <label htmlFor="unit-display" className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">
                    Unit
                  </label>
                  <input
                    id="unit-display"
              ...
              <div className="p-3.5 bg-slate-50 dark:bg-forest-800/40 border border-slate-200/50 dark:border-forest-700 rounded-2xl">
                ...
                {isRecurring && (
                  <div className="mt-3 animate-slide-up">
                    <label htmlFor="recurrence-period" className="sr-only">Recurrence period</label>
                    <select
              ...
              >
                {submitting ? (
                  <span className="flex items-center gap-2">
                    <span className="h-4 w-4 animate-spin rounded-full border-2 border-white border-t-transparent" aria-hidden="true"></span>
                    Saving&hellip;
                  </span>
                ) : (
                  'Log activity'
          ...
          <div className="p-4 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/20 border border-amber-100 dark:border-amber-800/60 rounded-2xl" role="region" aria-label="Reduction swap suggestion">
            <div className="flex items-start gap-2.5">
              <Lightbulb className="h-5 w-5 text-amber-500 shrink-0 mt-0.5" aria-hidden="true" />
              <div>
                <h3 className="text-xs font-bold text-amber-800 dark:text-amber-300">Simple Swap to Reduce Your Footprint</h3>
                <p className="mt-1 text-xs font-medium text-amber-700 dark:text-amber-400 leading-relaxed">
                  {category === 'transport' && subcategory.startsWith('car') &&
                  ...
                  {category === 'transport' && subcategory === 'walking' &&
                  ...
                  {category === 'transport' && subcategory === 'flights' &&
                  ...
                  {category === 'energy' && subcategory === 'lpg' &&
                    'Consider switching to renewable energy or improving home insulation to reduce heating fuel consumption.'}
                  {category === 'energy' && subcategory === 'solar' &&
                    'Great choice! Solar energy is a clean, renewable source. You are building toward a net-zero home.'}
                  {category === 'food' && subcategory === 'meat' &&
                    'Swapping one meat meal for a plant-based alternative saves ~4.6 kg CO₂e. Try it next time!'}
                  {category === 'food' && (subcategory === 'vegetarian' || subcategory === 'vegan') &&
                    'Great choice! Plant-based meals have a fraction of the carbon footprint of meat-based meals.'}
                  {category === 'shopping_waste' && (subcategory === 'shopping' || subcategory === 'deliveries') &&
                    'Try buying second-hand or borrowing instead. Extending product lifespans cuts manufacturing emissions by up to 50%.'}
                  {category === 'shopping_waste' && subcategory === 'recycling' &&
                    'Recycling is fantastic! You are reducing landfill methane and saving raw material extraction emissions.'}
                  {(!category || (!category.includes('transport') && !category.includes('energy') && !category.includes('food') && !category.includes('shopping'))) &&
                    'Every activity you track helps build awareness. Small daily changes add up to big reductions.'}
                </p>
              </div>
            </div>
          </div>

          <div className="bg-white dark:bg-forest-900 border border-slate-100 dark:border-forest-800 p-6 rounded-[28px] shadow-sm transition-colors duration-200" role="region" aria-label="Quick log presets">
              ...
              {smartSuggestions.map((sug, i) => (
                ...
                >
                  <span className="flex items-center gap-2.5 min-w-0">
                    <span className="p-1.5 bg-slate-100 dark:bg-forest-800 text-slate-600 dark:text-slate-400 rounded-lg shrink-0" aria-hidden="true">
          ...
          <div className="bg-white dark:bg-forest-900 border border-slate-100 dark:border-forest-800 rounded-[28px] shadow-sm p-6 transition-colors duration-200">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-slate-100 dark:border-forest-800">
              ...
              <div className="flex flex-wrap gap-2 items-center w-full sm:w-auto">
                <div className="relative flex-1 sm:flex-none">
                  <span className="absolute inset-y-0 left-0 flex items-center pl-2.5 text-slate-400 pointer-events-none" aria-hidden="true">
                    <Search className="h-4 w-4" />
                  </span>
                  <label htmlFor="search-logs" className="sr-only">Search subcategory</label>
                  <input
                    id="search-logs"
                    type="text"
            ...
            {loadingHistory ? (
              <div className="space-y-4 pt-4" role="status" aria-label="Loading activity history">
                ...
                <div className="h-10 bg-slate-50 dark:bg-forest-800/60 animate-pulse rounded-xl" aria-hidden="true"></div>
                <div className="h-10 bg-slate-100 dark:bg-forest-800 animate-pulse rounded-xl" aria-hidden="true"></div>
              ...
              <div className="overflow-x-auto pt-4">
                <table className="w-full text-left text-xs font-semibold text-slate-700 dark:text-slate-300" role="table" aria-label="Activity log history">
                  ...
                  <thead>
                    <tr className="text-slate-400 dark:text-slate-500 border-b border-slate-100 dark:border-forest-800">
                      <th className="py-3 pr-2 font-medium" scope="col">Category</th>
                      <th className="py-3 font-medium" scope="col">Subcategory</th>
                      <th className="py-3 text-right font-medium tabular-nums" scope="col">Quantity</th>
                      <th className="py-3 text-right font-medium tabular-nums" scope="col">CO&shy;e (kg)</th>
                      <th className="py-3 text-center font-medium" scope="col">Recurrence</th>
                      <th className="py-3 text-center font-medium" scope="col">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-50 dark:divide-forest-800/60">
                    {history.map((item) => (
                      <tr key={item.id} className="hover:bg-slate-50/50 dark:hover:bg-forest-800/40 transition-colors animate-fade-in">
                        <td className="py-3 pr-2">
                          <span className="flex items-center gap-2">
                            <span className="p-1 bg-slate-100 dark:bg-forest-800 text-slate-600 dark:text-slate-400 rounded-lg shrink-0" aria-hidden="true">
                        ...
                        <td className="py-3 text-right tabular-nums">{item.quantity} {item.unit}</td>
                        <td className="py-3 text-right tabular-nums">
                          {item.co2Emissions > 0 ? (
                            <span className="text-amber-700 dark:text-amber-400 font-bold">{item.co2Emissions} kg</span>
                          ) : item.co2Emissions < 0 ? (
                            <span className="text-emerald-600 dark:text-emerald-400 font-bold">-{Math.abs(item.co2Emissions)} kg</span>
                          ) : (
